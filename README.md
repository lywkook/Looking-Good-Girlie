...
5. **飲食紀錄 · 熱量(手動輸入)**:選餐別(早/午/晚/點心)→ 輸入食物;內建約 125 筆食物資料庫(`FOOD_DB`,以各種**食材**為主:各式肉類/海鮮/蛋豆、澱粉根莖、蔬菜、水果、乳製品、堅果油脂 + 台式外食),打字用 `<datalist>` 跳建議並自動帶入熱量 → 存進 `foodlog`(JSON array `{id,date,meal,name,kcal}`)。可設「今日熱量目標」(存 `kcaltarget`),即時算今日總熱量、進度條、還可吃/超過多少;`buildAdvice()` 依「今天吃多少 + 今天是重訓/燃脂/休息日 + 哪幾餐還沒吃」動態給飲食或運動的調整建議(超標建議減澱粉或加運動、額度足則正常吃)。
