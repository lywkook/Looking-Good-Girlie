...
## 設定頁(⚙️)
- 底部主選單第 4 顆「⚙️ 設定」開啟 modal(`#settingsModal`),集中管理:
  - **主題色**(這個計畫,存 `themecolor`):8 色**莫蘭迪色盤** + 系統取色器 + **直接輸入色號**(`#RRGGBB`);`--pink/--pink-soft/--pink-shadow` 由主色 `shade()` 自動推算。
  - **背景**(全域,存 `bgcolor`/`bgimage`):莫蘭迪色票 + 取色器 + 輸入色號 + **上傳自訂背景照片** + 清除。
  - **字體**(粗線體/紅豆奶霜體切換)。
  - **個人資料 · 減重分析**(只在 `full`/`origin` 計畫顯示;新增 full 計畫時自動開啟設定填寫)。
- 朋友版同樣可自行改主題色、背景照片/顏色(都是各自裝置本機儲存)。

## 分享給朋友(空白版)
- 資料存在**各自裝置的瀏覽器**(localStorage),不同手機互不相通 → 直接把網址傳給朋友,朋友打開就是他自己的空白紀錄。
- 網址加上 **`?blank`**(例:`https://<site>/?blank`)= **通用空白模式**:第一次開啟時的預設計畫改成中性的「我的計畫」(`full` 減重規劃,不帶回台專屬的倒數/菜單範本),適合送給朋友當起手式。偵測方式:`location.search+hash` 含 `blank` 且 storage 尚無 plans 時,預設 plan 用 `full`。
