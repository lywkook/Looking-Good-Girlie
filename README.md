...
5. **飲食紀錄 · 熱量(手動輸入)**:選餐別(早/午/晚/點心)→ 輸入食物;內建約 165 筆食物資料庫(`FOOD_DB`,以各種**食材**為主:各式肉類/海鮮/蛋豆、澱粉根莖、蔬菜、水果、乳製品、堅果油脂 + 台式外食),打字用**自訂建議清單**即時篩選、顯示名稱+熱量、點選帶入(取代 iOS 支援不佳的 `<datalist>`);資料庫沒有的加入後記成自訂食材(`customfoods`),下次會出現在建議 → 存進 `foodlog`(JSON array `{id,date,meal,name,kcal}`)。可設「今日熱量目標」(存 `kcaltarget`),即時算今日總熱量、進度條、還可吃/超過多少;`buildAdvice()` 依「今天吃多少 + 今天是重訓/燃脂/休息日 + 哪幾餐還沒吃」動態給飲食或運動的調整建議(超標建議減澱粉或加運動、額度足則正常吃)。
