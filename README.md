...
7. **多計畫(長期使用)**:頂部可切換/新增計畫,每個計畫類型不同會顯示不同區塊 —— `weight`(純體重紀錄=只有量測)、`calorie`(熱量紀錄=飲食+量測)、`full`(減重規劃=個人分析+飲食+量測)、`origin`(原本的完整回台計畫)。計畫清單存 `plans`、目前計畫存 `currentplan`;**各計畫資料獨立**(非預設計畫的 storage key 會加 `p:<planId>:` 前綴,例如 `p:pl123:metricslog`),`customfoods` 為跨計畫共用。可刪除自建計畫。
